...
int dfsz(int u, int par = -1) // caution: when using hld together, it must not overlap with the sz array used in hld
{	
	sz[u] = 1;
...
void upd(int v) // Change the color of node v of the centroid tree update.
{	
	color[v] = !color[v];
	int i = v;
	int ct = 0;
...
int query(int v) // Centroid tree query. Find the black vertex v and the shortest white vertex.
{
	int i = v;
	int ret = INF;
	int ct = 0;
